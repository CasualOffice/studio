...
fn main() {
    ...
    check!("create with a passphrase", vault.create("correct horse battery", false).is_ok());
    ...
    check!("content round-trips", vault.get(&first).unwrap() == b"identical bytes");
    ...
    check!(
        ...
        vault.list().unwrap().iter().any(|i| i.kind == "recovered" && i.mime == "image/png")
    );
    ...
    let report = vault.repair().unwrap();
    check!("foreign file is reported, not destroyed", report.unreadable == 1 && foreign.exists());

    let out = dir.join("exported.png");
    check!("export writes plaintext where asked", vault.export(&first, &out).is_ok());
    check!("exported bytes match", std::fs::read(&out).unwrap() == b"identical bytes");

    vault.lock();
    check!("locked vault refuses reads", vault.get(&first).is_err());
    check!("wrong passphrase is rejected", vault.unlock_with_passphrase("wrong").is_err());
    check!("correct passphrase reopens it", vault.unlock_with_passphrase("correct horse battery").is_ok());
    check!("items survived the lock cycle", vault.list().unwrap().len() == 3);

    check!("passphrase can be changed", vault.change_passphrase("correct horse battery", "a longer new one").is_ok());
    vault.lock();
    check!("new passphrase works", vault.unlock_with_passphrase("a longer new one").is_ok());
    check!("old passphrase no longer works", {
        vault.lock();
        let bad = vault.unlock_with_passphrase("correct horse battery").is_err();
    ...
    let _ = std::fs::remove_dir_all(&dir);
    println!();
    if failures == 0 { println!("VAULT LIFECYCLE OK"); }
